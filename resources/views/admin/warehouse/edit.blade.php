        ...
        <form action="{{ route('admin.warehouse.update', $warehouse->id) }}" method="POST">
          ...
          <div class="form-group">
            ...
            <select id="my-select" class="form-control @error('state') is-invalid @enderror" name="state">
              @foreach ($states as $state)
                <option value="{{ $state->state_id }}" @if ($state->state_id == $warehouse->state_id) selected @endif>
                  {{ $state->name }}</option>
              @endforeach
            </select>
          </div>
          ...
          <div class="form-group">
            ...
            <select id="my-select" class="form-control @error('status') is-invalid @enderror" name="status">
              <option value="1" @if ($warehouse->status == 1) selected @endif>activate</option>
              <option value="2" @if ($warehouse->status == 2) selected @endif>De-activate</option>
            </select>
          </div>
